  ...
  <div class="main">
    ...
    <div class="sign-up-container">
      <el-form ref="userForm" :model="params">
          <!-- 
            匹配结果：
            required：true  ：表示这个输入框必须有值
            message: '请输入你的昵称'：显示的提示信息
            trigger: 'blur'：在什么时候执行，blur：在失去焦点的是执行
              :rules="[
            {
              required: true,
              message: '请输入你的昵称',
        ...
        <!-- 
            required: true：不能为空
            message: '请输入手机号码'：错误的提示信息
            trigger: 'blur'： 在失去焦点时执行
            validator: checkPhone, trigger: 'blur'：checkPhone这个是自定义的方法，当失去焦点时执行自定义的方
            :rules="[
...
export default {
  ...
  methods: {
    ...
    //注册提交的方法
    submitRegister() {
        registerApi.saveUser(this.params).then(response => {
            //提示信息
            this.$message({
                type: 'success',
                message: '注册成功'
